...
namespace qlm
{
	...
	{
        VectorProcessor_Elementwise<mem_type, OpDiv>(in, val, dst);
	}

    template void Div<MemType::MEM_GPU>(const Vector<MemType::MEM_GPU>&, const Array<1, MemType::MEM_GPU>&, Vector<MemType::MEM_GPU>&);
    template void Div<MemType::MEM_UM>(const Vector<MemType::MEM_UM>&, const Array<1, MemType::MEM_UM>&, Vector<MemType::MEM_UM>&);
}