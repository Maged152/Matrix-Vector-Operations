...
struct VectorScalarAdd : ::testing::TestWithParam<std::tuple<
    int,   // length
    float, // min value
    float  // max value
    >>
...
TEST_P(VectorScalarAdd, Test_VectorScalarAdd)
{
    ...
    // run cpu code
    timer_cpu.Start();
    test::Add(src, val.Get(0), dst_cpu);
    timer_cpu.End();

    // run gpu code
    timer_gpu.Start();
    qlm::Add(src, val, dst_gpu);
    timer_gpu.End();
...
INSTANTIATE_TEST_CASE_P(
    Test_VectorScalarAdd, VectorScalarAdd,
    ::testing::Combine(
