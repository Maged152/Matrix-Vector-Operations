...
struct VectorMul : ::testing::TestWithParam<std::tuple<
    int,   // length
    float, // min value
    float  // max value
    >>
...
TEST_P(VectorMul, Test_VectorMul)
{
    ...
    // run cpu code
    timer_cpu.Start();
    test::Mul(src1, src2, dst_cpu);
    timer_cpu.End();

    // run gpu code
    timer_gpu.Start();
    qlm::Mul(src1, src2, dst_gpu);
    timer_gpu.End();
